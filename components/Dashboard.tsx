...
const Dashboard = () => {
  ...
  const handleToggle = (section: keyof typeof tasks, id: number) => {
    const updated = { ...tasks };
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {Object.keys(tasks).map((section) => {
        const completed = tasks[section as keyof typeof tasks].filter((t) => t.done).length;
        const total = tasks[section as keyof typeof tasks].length;
      ...
      <div className="task-section">
        {Object.entries(tasks).map(([section, sectionTasks]) => (
          <TaskCard key={section} section={section} tasks={sectionTasks} onToggle={handleToggle} />
        ))}
      </div>
